...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inverse(bd_sizes[k].alloc, bi / 2);      // xor p's pair at size k
...
int bd_initfree_pair(int k, int bi, int pos) {
  ...
  if (bit_isset(bd_sizes[k].alloc, bi / 2)) {
    free = BLK_SIZE(k);
    if (pos == LEFT_PAIR) {
      lst_push(&bd_sizes[k].free, addr(k, (bi % 2 == 0) ? bi + 1 : bi));
      // for left pair free may be only right buddy
    } else {
      lst_push(&bd_sizes[k].free, addr(k, (bi % 2 == 0) ? bi : bi - 1));
      // for right pair free may be only left buddy
    }
  }
  return free;
}
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    free += bd_initfree_pair(k, left, LEFT_PAIR);
    if (right <= left) continue;
    free += bd_initfree_pair(k, right, RIGHT_PAIR);
  }
  return free;
}
